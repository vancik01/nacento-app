	...
	if (valueId === "total" || valueId === "total_vat") {
		...
		sections.map((section, sectionId) => {
			if (parseFloat(section.info.total) !== 0) {
				...
				console.log(add_construction, add_delivery);
			}
		});
	} else {
		sections.map((section, sectionId) => {
...
export function splitBetweenItems(block, value, valueId) {
	...
	if (block.items.length == 0) {
		if (valueId == "total") {
			...
			block.info[valueId] = round(
				parseFloat(block.info[valueId]) + parseFloat(value),
				2
			);
...
export function updateTableRow(item, valueId, value) {
	...
	if (valueId === "total_construction_price") {
		...
		item["unit_delivery_price"] = round(
			item["total_delivery_price"] / item["quantity"],
			2
		);

		item.total_delivery_price = round(value, 2);
		...
		item.total_construction_price = round(
			parseFloat(item.total_construction_price) +
				parseFloat(valueChange * cmcIndex),
			2
		);

		item.total_delivery_price = round(
			parseFloat(item.total_delivery_price) +
				parseFloat(valueChange * cdcIndex),
			2
		);

		item.unit_construction_price = round(
		...
		item.total = round(parseFloat(value), 2);
	} else {
		item["total_construction_price"] = round(
		...
		item["total_delivery_price"] = round(
			item.unit_delivery_price * item.quantity,
			2
		);

		item.total = round(
			item.total_construction_price + item.total_delivery_price,
			2
		);
...
export function updateTotals(data) {
	...
	data.data.totals["total_construction_price"] = 0;
	data.data.totals["total_delivery_price"] = 0;

	data.data.sections?.map((section, sectionId) => {

		data.data.totals.total += parseFloat(section.info["total"]);
		data.data.totals["total_construction_price"] += parseFloat(section.info["total_construction_price"]);
		data.data.totals["total_delivery_price"] += parseFloat(section.info["total_delivery_price"]);
	});
