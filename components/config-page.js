		...
		<Layout className=" text-center">
				...
				{data.map((block1, i) => {
					return (
						<div className="flex flex-col items-center gap-3">
							<div className="mt-16">
								...
								{i < images.length ? (
									<div className="text-xl">
										{" "}
										<ThreejsView label={block1.label} image={images[i]} />
									</div>
								) : (
									<div className="text-xl">
										{" "}
										<ThreejsView label={block1.label} />
									</div>
								)}
							...
							{i === 0 && (
								<FileButton title={"Pôdorys základov"} toSend={setZaklady} />
							)}
							{i === 1 && (
								<FileButton title={"Pôdorys NP"} toSend={setPodorysy} />
							)}
							{i === 2 && (
								<FileButton title={"Pôdorys strechy"} toSend={setStrecha} />
							)}
						</div>
					);
				})}
			...
			{(zakaldyPdf || podorysyPdf.length || strechaPdf) && active && (
				<button
					...
					className={`py-4 px-8 hover:bg-gray-50 border rounded-md  justify-center gap-2 text-start `}
				>
					{" "}
					Vyplň údaje{" "}
				</button>
			...
			<div className="flex justify-center mt-20 gap-2">
				<button
					...
					className={`py-4 px-8 bg-gray-50 hover:bg-transparent	 border rounded-md`}
				>
		...
		<>
			{!fileName && (
				<>
					...
					<button
						onClick={handleClick}
						className={`py-2 px-8 hover:bg-gray-50 border rounded-md`}
					>
						{props.title}
					</button>
				</>
