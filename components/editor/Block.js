...
export default function Block({
	...
	const { editBlockTitle, addTableRow, reorderingBlocks, deleteBlock } = useData();
...
function BlockTotals({ blockId, sectionId }) {
	const [total] = getValue(
		(data) => data.data.sections[sectionId].blocks[blockId].info.total
	);
	const [total_delivery_price] = getValue(
		(data) => data.data.sections[sectionId].blocks[blockId].info.total_delivery_price
	);
