		...
		for (let s = 0; s < CP.sections.length; s++) {
			for (let b = 0; b < CP.sections[s].blocks.length; b++) {
				...
				for (let i = 0; i < CP.sections[s].blocks[b].items.length; i++) {
					...
					let unit_delivery_price =
						CP.sections[s].blocks[b].items[i].unit_delivery_price;
					let unit_construcion_price =
						CP.sections[s].blocks[b].items[i].unit_construction_price;

					CP.sections[s].blocks[b].items[i].total_delivery_price = round(
						quantity * unit_delivery_price
					);
					CP.sections[s].blocks[b].items[i].total_construction_price = round(
						quantity * unit_construcion_price
					);

					block_delivery_total +=
						CP.sections[s].blocks[b].items[i].total_delivery_price;
					block_construction_total +=
						CP.sections[s].blocks[b].items[i].total_construction_price;
				}
				CP.sections[s].blocks[b].info.total_construction_price =
					block_construction_total;
				CP.sections[s].blocks[b].info.total_delivery_price =
					block_delivery_total;
			}
		}
	}
