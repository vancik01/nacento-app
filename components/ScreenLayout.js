            ...
            <div className='grid' style={{gridTemplateColumns:"auto 1fr"}}>
                <div className='w-fit relative h-screen'>
                    <AnimatePresence mode='wait'>
                        {displaySidebar &&
                            <motion.div key={"sidebar"} className="" initial={{x:-300, width:0}} animate={{x:0,width:300}} exit={{x:-300, width:0}}>
                                <Sidebar></Sidebar>
                            </motion.div>
                        }
                        {!displaySidebar&&<div></div>}
                        {!displaySidebar&&<motion.button initial={{x:-50}} animate={{x:0}} onClick={()=>{setdisplaySidebar(!displaySidebar)}} className='fixed top-12 p-3 left-0 z-50 bg-white shadow-lg'>
